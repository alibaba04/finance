...
//Jika Mode Hapus/Delete
    if ($_GET["txtMode"] == "Delete") {
        $pesan = $tmpJurnalUmum->delete($_GET["kode"],$_GET["desc"]);
    }
    ...
    function omodal(param) {
        ...
        $('#btnDelete').click(function(){
            if($("#txtDelete").val()!= ''){
                ...
                window.location.href='index2.php?page=view/jurnalumum_list&txtMode=Delete&kode='+param+'&desc='+desc;
            }else{
...
//database
                ...
                $q.= " ORDER BY j.kode_transaksi asc,t.keterangan_transaksi,t.debet desc";
//Paging
                            ...
                            while ($query_data = $rs->fetchArray()) {
                                ...
                                if ($hakUser == 90) {
                                    ...
                                    if(empty($query_data["keterangan_posting"])){
                                        echo "<td><a class='label label-success' style='cursor:pointer;' onclick=location.href='" . $_SERVER['PHP_SELF'] . "?page=view/jurnalumum_detail&mode=edit&kode=" . md5($query_data["kode_transaksi"]) . "'><i class='fa fa-edit'></i>&nbsp;Update</span></td>";
                                        ...
                                        if ($_SESSION["my"]->privilege == 'GODMODE') {
                                            echo "<td><span class='label label-success' style='cursor:pointer;' onclick=location.href='" . $_SERVER['PHP_SELF'] . "?page=view/jurnalumum_detail&mode=edit&kode=" . md5($query_data["kode_transaksi"]) . "'><i class='fa fa-edit'></i>&nbsp;Update</span></td>";
